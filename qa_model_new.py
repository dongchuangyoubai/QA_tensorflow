...
class Encoder(object):
    def __init__(self, config, initializer=lambda: None):#tf.contrib.layers.xavier_initializer):
    ...
    def encode(self, inputs, masks, encoder_state_input = None):
        ...
        with tf.variable_scope("encoded_question"):
            ...
            # lstm_cell_fw_question = tf.contrib.rnn.BasicLSTMCell(self.hidden_size, state_is_tuple=True)
            # lstm_cell_bw_question = tf.contrib.rnn.BasicLSTMCell(self.hidden_size, state_is_tuple=True)
            #
            # # dropout
            # lstm_cell_fw_question = tf.contrib.rnn.DropoutWrapper(cell=lstm_cell_fw_question,
            #                                                       output_keep_prob=self.keep_prob)
            # lstm_cell_bw_question = tf.contrib.rnn.DropoutWrapper(cell=lstm_cell_bw_question,
            #                                                       output_keep_prob=self.keep_prob)
            #
            # # multi lstm layers
            # cell_fw = tf.contrib.rnn.MultiRNNCell([lstm_cell_fw_question] * self.num_layers, state_is_tuple=True)
            # cell_bw = tf.contrib.rnn.MultiRNNCell([lstm_cell_bw_question] * self.num_layers, state_is_tuple=True)
            #
            # # initial state
            ...
            #
            # encoded_question_, _, _ = tf.nn.static_bidirectional_rnn(cell_fw=lstm_cell_fw_question,
            #                                                         cell_bw=lstm_cell_bw_question,
            #                                                         inputs=question,
            #                                                         initial_state_fw=initial_state_fw,
            #                                                         initial_state_bw=initial_state_bw,
            #                                                         dtype=tf.float32)
            # encoded_question = tf.reshape(tf.concat(encoded_question_, 1), [-1, self.hidden_size * 2])
            lstm_cell_question = tf.contrib.rnn.BasicLSTMCell(self.hidden_size, state_is_tuple=True)
            encoded_question, (q_rep, _) = tf.nn.dynamic_rnn(lstm_cell_question, passage, masks_passage, dtype=tf.float32)
...
class QASystem(object):
    ...
    def setup_system(self):
        """
        Apply the encoder to the question and passage embeddings.
        Follow that up by Match-LSTM and Answer-Ptr
        """
        encoded_question, encoded_passage, q_rep, p_rep = self.encoder.encode(
                                                                [self.question, self.passage],
                                                                [self.question_lengths, self.passage_lengths],
                                                                encoder_state_input=None)

        if self.config.use_match:
            self.logger.info("\n========Using Match LSTM=========\n")
            self.logits = self.decoder.decode([encoded_question, encoded_passage], q_rep
                                         [self.question_lengths, self.passage_lengths], self.labels)
        else:
            self.logger.info("\n========Using Vanilla LSTM=========\n")
            self.logits = self.decoder.decode_lstm([encoded_question, encoded_passage], q_rep
                                                   [self.question_lengths, self.passage_lengths], self.labels)
